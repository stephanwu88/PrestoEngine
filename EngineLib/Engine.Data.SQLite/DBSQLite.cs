...
namespace Engine.Data.SQLite
{
    /// <summary>
    /// SQLite数据库类
    /// </summary>
    public class DBSQLite : IDBFactory<LocalSource>
    {
        #region 内部成员
        ...
        /// DataReader对象
        /// </summary>
        public IDataReader DbDataReader => _SqliteDataReader;
        /// <summary>
        /// 连接状态
        /// </summary>
        ...
        public DBSQLite(LocalSource localSource = null)
        {
            if (_SqliteConn == null)
            {
                _SqliteConn = new SQLiteConnection();
        ...
        public IDbConnection CreateConnection()
        {
            try
            {
                ...
                {
                    if (_SqliteConn.ConnectionString != _ConnString)
                        _SqliteConn.ConnectionString = _ConnString;
                    _SqliteConn.Open();
                }
                ChangeState(_SqliteConn.State);
                //if (!StateConnected)
            ...
            catch (SQLiteException ex)
            {
                if (_LastError != ex.Message)
                {
                    ...
                    Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【SQLite Connect Fail】 {0}", ex.Message));
                }
                ChangeState(_SqliteConn.State);
                //if (!StateConnected)
                //    ThreadPool.QueueUserWorkItem(WatchConnection, null);
        ...
        public IDataReader CreateDataReader(string strSql)
        {
            ...
            if (_SqliteCommand != null)
            {
                _SqliteCommand.CommandText = strSql;
                dr = _SqliteCommand.ExecuteReader();
            }
            return dr;
        }
        ...
        public IDataReader Query(string strSql)
        {
            ...
            return ret.Success ? _SqliteDataReader : null;
        }
        ...
        public CallResult CmdByTran(List<string> ListSql)
        {
            ...
            {
                CreateCommand();
                SQLiteTransaction tran = _SqliteConn.BeginTransaction();
                _SqliteCommand.Transaction = tran;
                try
                {
                    int count = 0;
                ...
                catch (Exception ex)
                {
                    ...
                    if (_LastError != ex.Message)
                    {
                        Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【SQLite TranError】 {0}", ex.ToString()));
                        _LastError = ex.Message;
                    }
                    tran.Rollback();
                    return ret;
        ...
        {
            if (ConnectionStateChanged != null && !_ConnStateTemp.Equals(_newState))
            {
        ...
        private void CreateConnectString(LocalSource localSource)
        {
            if (localSource == null)
            {
                ...
                RaiseError(strErr);
                Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【SqliteFile Connect Fail】 {0}", strErr));
                return;
            }
            ...
            if (!string.IsNullOrEmpty(localSource.Password))
            {
                conStr = new SQLiteConnectionStringBuilder
                {
                    DataSource = localSource.SourceFile,
                    Password = localSource.Password,
                    //Version = 3,
                };
            }
            else
        ...
        private CallResult CmdBySql(string method, string strSql, CommandType type = CommandType.Text)
        {
            lock (_SqliteConn)
            {
                ...
                {
                    CreateCommand();
                    _SqliteCommand.CommandType = type;
                    _SqliteCommand.CommandText = strSql;
                    ...
                    switch (method.ToUpper())
                    {
                        case "SELECT":
                            ...
                            dt.Load(_SqliteDataReader);
                            ret.Result = dt;
                            ret.Success = true;
                            break;
                        case "EXE":
                            ret.Result = _SqliteCommand.ExecuteNonQuery();
                            ret.Success = true;
                            break;
                        default:
                            ret.Result = _SqliteCommand.ExecuteNonQuery();
                ...
                catch (Exception ex)
                {
                    ...
                    if (_LastError != ex.Message)
                    {
                        Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【SQLite CmdError】\r\n ErrMsg:\t\t{0} \r\nSqlStatement:\t{1}", ex.Message, strSql));
                        _LastError = ex.Message;
                    }
                }
                return ret;
