...
namespace Engine.WpfControl
{
    ...
    public class DrapAdornerBehavior : Behavior<UIElement>
    {
        ...
        private void AssociatedObject_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            ...
            if (e.LeftButton == MouseButtonState.Pressed)
            {
                    ...
                    Math.Abs(diff.Y) > SystemParameters.MinimumVerticalDragDistance)
                {
                    ...
                    var control = this.AncestorType == null ? element : element.GetParent(this.AncestorType) as UIElement;
                    control = control == null ? element : control;

                    var layer = AdornerLayer.GetAdornerLayer(control);

                    var find = layer.GetAdorners(element)?.Where(l => l == adorner);
        ...
        {
            var dragData = new DataObject(this.DragGroup, adorner);
    ...
    public class DrogGridAdorner : Adorner
    {
