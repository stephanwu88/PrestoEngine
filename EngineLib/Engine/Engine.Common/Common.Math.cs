...
namespace Engine.Common
{
    ...
    public partial class sCommon
    {
        ...
        /// <returns></returns>
        public static double CalculateMean(this List<double> data)
        {
            try
            {
        ...
        /// </summary>
        /// <returns></returns>
        public static double CalculateVariance(this List<double> data)
        {
            int n = data.Count - 1;
            // 计算平均值
            double mean = data.Average();
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static double CalculateStandardDeviation(this List<double> data)
        {
            int n = data.Count - 1;
            // 计算平均值
            double mean = data.Average();
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static double CalculateRelativeStandardDeviation(this List<double> data)
        {
            int n = data.Count - 1;
            //计算平均值
            double mean = data.Average();
            //每个数值与平均值的差的平方的总和
            double sumOfSquaredDeviations = data.Sum(d => Math.Pow(d - mean, 2));
            //并将其除以自由度（数据个数减1），得到 标准偏差
            double standardDeviation = Math.Sqrt(sumOfSquaredDeviations / n);
            //将标准偏差除以平均值，并乘以100
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static double CalculateRange(this List<double> data)
        {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static double ToMyLgData(this double data)
        {
            double result = Math.Log10(data);
            return result;
        }
        ...
        /// 反lgn 计算n值
        /// </summary>
        /// <param name="dataOfLgn">lg值</param>
        /// <param name="logButtom">lg底数</param>
        /// <returns></returns>
        public static double InvLgData(this double dataOfLgn, int logButtom = 10)
        {
            double n = Math.Pow(logButtom, dataOfLgn);
            return n;
        }
        ...
        /// <param name="decimalPlaces">保留小数</param>
        /// <returns></returns>
        public static double RoundToDecimal(this double number, int decimalPlaces)
        {
            double roundedValue = Math.Round(number,decimalPlaces);
            return roundedValue;
        }
        ...
        /// <returns></returns>
        public static string RoundToDecimalAndFormat(this string strNumber, int decimalPlaces, string DataFormat = "0.0#")
        {
            double number = strNumber.ToMyDouble();
            return number.RoundToDecimalAndFormat(decimalPlaces,DataFormat);
        }
        ...
        /// ex:0.0## 0: 补零占位  #:省略零占位符 最多显示两位小数，并自动忽略末尾的零
        /// </param>
        /// <returns></returns>
        public static string RoundToDecimalAndFormat(this double number, int decimalPlaces, string DataFormat = "0.0#")
        {
            try
            {
                //遇到科学计数，先格式化再修约
                if (number.IsScientificNotation())
                {
                    string strNumber = number.ToString("F10");
                    double.TryParse(strNumber, out number);
                }
                double roundedValue = Math.Round(number, decimalPlaces);
                string strRoundedValue = roundedValue.ToString(DataFormat);
        ...
        /// <param name="number"></param>
        /// <returns></returns>
        public static double RoundToDn(this double number)
        {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static bool IsScientificNotation(this double data)
        {
            return data.ToString().ToLower().Contains("e");  // 判断是否存在指数符号
        }
        ...
        /// <param name="NearestIndex">最近的数据索引</param>
        /// <returns></returns>
        public static double GetNearestValue(this double dataToCheck, List<double> TargetData, out int NearestIndex)
        {
            NearestIndex = -1;
            if (TargetData.MyCount() == 0)
                return SystemDefault.InValidDouble;
            double minDifference = double.MaxValue;
            NearestIndex = 0;
            for (int i = 0; i < TargetData.Count; i++)
            {
                double difference = Math.Abs(TargetData[i] - dataToCheck);
            ...
            return TargetData.MySelectAny(NearestIndex);
        }
        ...
        /// <returns></returns>
        public static string ConvertToRegularNotation(this string strData)
        {
            double data = strData.ToMyDouble();
            return data.ConvertToRegularNotation();
        }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static string ConvertToRegularNotation(this double data)
        {
            string strData = data.ToString("F10");
            return strData;
            //if (!IsScientificNotation(data))  
            //    return data.ToString();
            //strData = strData.ToLower();
            //// 拆分底数和指数部分 ex: 6.022E+23
            //string[] parts = strData.ToLower().Split('e');  
            //double baseValue = double.Parse(parts[0]);
            //int exponent = int.Parse(parts[1]);
            //return (baseValue * Math.Pow(10, exponent)).ToString("F0");  // 计算常规计数
        }
    }
