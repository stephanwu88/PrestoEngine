...
namespace Engine.MVVM.Messaging
{
    /// <summary>
    ...
    public class Messenger : IMessenger
    {
        ...
        {
            var classType = typeof(TMessage);
            if (_recipientsStrictAction == null) return false;
            return _recipientsStrictAction.ContainsKey(classType);
        }
        ...
        {
            if (!IsRegistered<TMessage>()) return false;
            var classType = typeof(TMessage);
            List<WeakActionAndToken> lists = _recipientsStrictAction[classType];
            foreach (var item in lists)
            {
                var weakAction = (IExecuteWithObject)item.Action;
                if (weakAction != null && recipient.MyEquals(weakAction.Target))
                {
                    return true;
                }
            }
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public bool IsRegistered<TMessage>(object recipient, object token)
        {
            if (!IsRegistered<TMessage>()) return false;
            var classType = typeof(TMessage);
            List<WeakActionAndToken> lists = _recipientsStrictAction[classType];
        ...
        /// <returns></returns>
        private void RemoveRegistered<TMessage>(object recipient, object token)
        {
            var classType = typeof(TMessage);
            if (_recipientsStrictAction == null) return;
            if (!_recipientsStrictAction.ContainsKey(classType)) return;
            List<WeakActionAndToken> lists = _recipientsStrictAction[classType];
            foreach (var item in lists)
            {
                var weakAction = (IExecuteWithObject)item.Action;
                bool TokenMatch = token == null ? true : item.Token.MyEquals(token);
                bool RecipientMatch = recipient == null ? true : weakAction != null && recipient.MyEquals(weakAction.Target);
                if (TokenMatch && RecipientMatch)
                {
                    Unregister<TMessage>(weakAction.Target,item.Token);
                    return;
                }
            }
        }
        ...
        public virtual void SingleRegister<TMessage>(object recipient, Action<TMessage> action)
        {
            RemoveRegistered<TMessage>(recipient, null);
            Register(recipient, null, false, action);
        }
        ...
        /// <param name="action"></param>
        public virtual void SingleRegister<TMessage>(object recipient, bool receiveDerivedMessagesToo, Action<TMessage> action)
        {
            RemoveRegistered<TMessage>(recipient,null);
            Register(recipient, null, receiveDerivedMessagesToo, action);
        }
        ...
        /// <param name="token"></param>
        /// <param name="action"></param>
        public virtual void SingleRegister<TMessage>(object recipient, object token, Action<TMessage> action)
        {
            RemoveRegistered<TMessage>(recipient, token);
            Register(recipient, token, false, action);
        }
        ...
        private static void UnregisterFromLists(object recipient, Dictionary<Type, List<WeakActionAndToken>> lists)
        {
            ...
            lock (lists)
            {
                ...
                {
                    List<WeakActionAndToken> RemoveList = new List<WeakActionAndToken>();
