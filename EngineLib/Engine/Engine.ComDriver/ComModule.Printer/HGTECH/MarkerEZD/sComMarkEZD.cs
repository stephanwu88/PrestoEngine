...
namespace Engine.ComDriver.HGTECH
{
    ...
    /// 北京金橙子激光镭射控制
    /// </summary>
    public class sComMarkEZD : sComMarkEZD<ServerNode>
    {
        public sComMarkEZD() : base()
        {
        ...
        public sComMarkEZD(DriverItem<NetworkCommParam> DrvItem) : base(DrvItem)
        {
    ...
    /// 北京金橙子激光镭射控制
    /// </summary>
    public partial class sComMarkEZD<TConNode> : sComRuleAsc where TConNode : new()
    {
        ...
        public HGMarker Marker = new HGMarker();
        ...
        public sComMarkEZD()
        {
            ...
            Marker.StateChanged += Marker_StateChanged;
        }
        ...
        public void Print(string ModelEzdFile,string PrintText)
        {
            Task.Factory.StartNew(() =>
            {
                ...
                Logger.Default["MES"].Write(LOG_TYPE.MESS, string.Format("[{0}]正在标刻...", PrintText));
                bool ret = Marker.LoadEzdFile(ModelEzdFile);
                List<string> LstPrint = PrintText.MySplit(",");
                if (LstPrint.MyCount() > 0)
                    ret = Marker.ChangeTextByName("CODE", LstPrint[0]);
                if (LstPrint.MyCount() > 1)
                    ret = Marker.ChangeTextByName("CODE2", LstPrint[1]);
                //ret = HGMarker.Instance.WpfShowPreviewBmp(_EzdView, _border.ActualWidth, _border.ActualHeight);
                ImageSource img = Marker.PreviewImageSource(200, 200);
                Marker.EzdView = img;
                //Session.EzdView = img;
                //Action<ImageSource> dd = new Action<ImageSource>((item) =>
                //{
                //    _EzdView.Source = item;
                //});
                //Dispatcher.Invoke(dd, img);
                if (ret) Marker.Mark(false);
                Logger.Default["MES"].Write(LOG_TYPE.MESS, string.Format("[{0}]标刻完成", PrintText));
            });
        }
            ...
            switch (strMessKey)
            {
                ...
                case "REGISTER":
                    ...
                    if (strStartNow == "Y")
                    {
                        ...
                        Print(sCommon.GetStartUpPath() + @"\Model\模板.ezd", strFields);
                    }
                    break;
                case "PROCESS_START":
                    break;
        ...
        {
            string AckContent = string.Empty;
            AckField._Status_Initializing = Marker.Initializing ? "1" : "0";
            AckField._Status_InitSuccessed = Marker.InitSuccessed ? "1" : "0";
            AckField._Status_Marking = Marker.Marking ? "1" : "0";
