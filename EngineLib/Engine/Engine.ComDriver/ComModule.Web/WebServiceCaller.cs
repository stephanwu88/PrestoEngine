...
namespace Engine.ComDriver
{
    ...
    public class WebServiceCaller
    {
        ...
        {
            var webresult = new WebServiceResult();
            XmlDocument xmldoc = null;
            try
            {
                if (_xmlNamespaces.ContainsKey(URL))
                {
                    xmldoc = QuerySoapWebService(URL, MethodName, Pars, _xmlNamespaces[URL].ToString());
                }
                else
                {
                    xmldoc = QuerySoapWebService(URL, MethodName, Pars, GetNamespace(URL));
                }
                if (xmldoc == null)
                {
                    webresult.Code = false;
                    webresult.Message = "WebService调用失败，没有回抛信息";
                    return webresult.Message;
                }
                else
                {
                    webresult.Message = xmldoc.InnerText;   // .GetJsonT<DataSendStruct>();
                }
            }
            catch (Exception ex)
            {
