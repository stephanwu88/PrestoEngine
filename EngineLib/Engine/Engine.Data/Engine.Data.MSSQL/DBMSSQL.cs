...
namespace Engine.Data.MSSQL
{
    /// <summary>
    /// MSSQL数据库类
    /// </summary>
    public class DBMSSQL : IDBFactory<ServerNode>
    {
        #region 内部成员
        ...
        /// DataReader对象
        /// </summary>
        public IDataReader DbDataReader => _SqlDataReader;
        /// <summary>
        /// 连接状态
        /// </summary>
        ...
        public DBMSSQL(ServerNode serverNode = null)
        {
            if (_SqlConn == null)
            {
                _SqlConn = new SqlConnection();
        ...
        public IDbConnection CreateConnection()
        {
            try
            {
                ...
                {
                    if (_SqlConn.ConnectionString != _ConnString)
                    {
            ...
            catch (SqlException ex)
            {
                if (_LastError != ex.Message)
                {
                    ...
                    Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【MSSql Connect Fail】 {0}", ex.Message));
                }
                ChangeState(_SqlConn.State);
                //if (!StateConnected)
                //    ThreadPool.QueueUserWorkItem(WatchConnection, null);
        ...
        public void CloseConnection()
        {
            if (_SqlConn.State != ConnectionState.Closed)
            {
                _SqlConn.Close();
                _SqlConn.Dispose();
                SqlConnection.ClearPool(_SqlConn);
            }
            ChangeState(_SqlConn.State);
        }
        ...
        public IDbCommand CreateCommand()
        {
            CreateConnection();
            if (_SqlCommand == null)
                _SqlCommand = _SqlConn.CreateCommand();
            return _SqlCommand;
        }
        ...
        public IDataReader Query(string strSql)
        {
            ...
            return ret.Success ? _SqlDataReader : null;
        }
        ...
        public CallResult CmdByTran(List<string> ListSql)
        {
            ...
            lock (_SqlConn)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (_LastError != ex.Message)
                    {
                        Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【MSSql TranError】 {0}", ex.ToString()));
                        _LastError = ex.Message;
                    }
                    tran.Rollback();
                    return ret;
        ...
        private void CreateConnectString(ServerNode serverNode)
        {
            ...
            if (serverNode.ServerPort == 0)
                serverNode.ServerPort = 1433;
            if (string.IsNullOrEmpty(serverNode.UserID))
                serverNode.UserID = "sa";
            ...
            _ConNode = serverNode;
            _ConnString = string.Format("Server={0},{1};DataBase={2};Uid={3};Pwd={4};pooling=false",
               serverNode.ServerIP,
        ...
        private CallResult CmdBySql(string method, string strSql, CommandType type = CommandType.Text)
        {
            lock (_SqlConn)
            {
                ...
                try
                {
                    ...
                    switch (method.ToUpper())
                    {
                        case "SELECT":
                            ...
                            break;
                        case "EXE":
                            ret.Result = _SqlCommand.ExecuteNonQuery();
                            ret.Success = true;
                            break;
                        default:
                            ret.Result = _SqlCommand.ExecuteNonQuery();
                ...
                catch (Exception ex)
                {
                    ...
                    if (_LastError != ex.Message)
                    {
                        Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【MsSql CmdError】\r\n ErrMsg:\t\t{0} \r\nSqlStatement:\t{1}", ex.Message, strSql));
                        _LastError = ex.Message;
                    }
                }
                return ret;
