...
namespace Engine.Data.MySQL
{
    ...
    /// MySQL数据库类
    /// </summary>
    public class DBMySQL : IDBFactory<ServerNode>
    {
        #region 内部成员
        ...
        /// DataReader对象
        /// </summary>
        public IDataReader DbDataReader => _MySqlDataReader;
        /// <summary>
        /// 连接状态
        /// </summary>
        ...
        public DBMySQL(ServerNode serverNode = null)
        {
            if (_MySqlConn == null)
            {
                _MySqlConn = new MySqlConnection();
        ...
        public IDbConnection CreateConnection()
        {
            try
            {
                ...
                {
                    if (_MySqlConn.ConnectionString != _ConnString)
                        _MySqlConn.ConnectionString = _ConnString;
                    _MySqlConn.Open();
                }
                ChangeState(_MySqlConn.State);
                //if (!StateConnected)
            ...
            catch (MySqlException ex)
            {
                if (_LastError != ex.Message)
                {
                    ...
                    Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【Mysql Connect Fail】 {0}", ex.Message));
                }
                ChangeState(_MySqlConn.State);
                //if (!StateConnected)
                //    ThreadPool.QueueUserWorkItem(WatchConnection, null);
        ...
        public void CloseConnection()
        {
            if (_MySqlConn.State != ConnectionState.Closed)
            {
                _MySqlConn.Close();
                _MySqlConn.Dispose();
                MySqlConnection.ClearPool(_MySqlConn);
            }
            ChangeState(_MySqlConn.State);
        }
        ...
        public IDbCommand CreateCommand()
        {
            ...
            if (_MySqlCommand == null)
                _MySqlCommand = _MySqlConn.CreateCommand();
        ...
        public IDataReader CreateDataReader(string strSql)
        {
            ...
            if (_MySqlCommand != null)
            {
                _MySqlCommand.CommandText = strSql;
                dr = _MySqlCommand.ExecuteReader();
            }
            return dr;
        }
        ...
        public IDataReader Query(string strSql)
        {
            ...
            return ret.Success ? _MySqlDataReader : null;
        }
        ...
        public CallResult SqlInsert<T>(T data, InsertMode InsertMode = InsertMode.None)
        {
            ...
            {
                string strInsertHead = "insert into ";
                if (InsertMode == InsertMode.IngoreInsert)
                    strInsertHead = "insert ignore into ";
                else if (InsertMode == InsertMode.ReplaceInsert)
                    strInsertHead = "replace into ";
                Res.Success = true;
                Res.Result = string.Format("{0}{1}({2}) values({3});", strInsertHead, strTableName, strInsertField, strInsertValue);
            }
            return Res;
        }
        ...
        public CallResult CmdByTran(List<string> ListSql)
        {
            ...
            {
                CreateCommand();
                MySqlTransaction tran = _MySqlConn.BeginTransaction();
                _MySqlCommand.Transaction = tran;
                try
                {
                    int count = 0;
                ...
                catch (Exception ex)
                {
                    ...
                    if (_LastError != ex.Message)
                    {
                        Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【Mysql TranError】 {0}", ex.ToString()));
                        _LastError = ex.Message;
                    }
                    tran.Rollback();
                    return ret;
        ...
        private void CreateConnectString(ServerNode serverNode)
        {
            ...
            _ConnString = string.Format("Server={0};Port={1};DataBase={2};User={3};Pwd='{4}';" +
                "SslMode=none;Pooling=False;Allow User Variables=True;Convert Zero Datetime=True;",
                serverNode.ServerIP,
        ...
        /// <param name="strSql"></param>
        /// <returns></returns>
        private string ConvertToSafeUpdate(string strSql)
        {
            string strSaftUpd = string.Empty;
            string strRet = strSql.ToLower();
            if (!strRet.Replace(" ", "").Contains("sql_safe_updates=0") && (strRet.Contains("update") || strRet.Contains("delete")))
                strSaftUpd = "set sql_safe_updates=0;\r\n";
            return strSaftUpd + strSql;
        }
        ...
        private CallResult CmdBySql(string method, string strSql, CommandType type = CommandType.Text)
        {
            ...
            lock (_MySqlConn)
            {
                try
                {
                    ...
                    _MySqlCommand.CommandText = ConvertToSafeUpdate(strSql);
                    ...
                    switch (method.ToUpper())
                    {
                        case "SELECT":
                            ...
                            dt.Load(_MySqlDataReader);
                            ret.Result = dt;
                            ret.Success = true;
                            break;
                        case "EXE":
                            ret.Result = _MySqlCommand.ExecuteNonQuery();
                            ret.Success = true;
                            break;
                        default:
                            ret.Result = _MySqlCommand.ExecuteNonQuery();
                ...
                catch (Exception ex)
                {
                    ...
                    if (_LastError != ex.Message)
                    {
                        Logger.Error.Write(LOG_TYPE.ERROR, string.Format("【Mysql CmdError】\r\n ErrMsg:\t\t{0} \r\nSqlStatement:\t{1}", ex.Message, strSql));
                        _LastError = ex.Message;
                    }
                }
            }
        ...
        private static bool GetFieldValue<T>(T data, PropertyInfo pi,
            ref ColumnDef colDef, ref string strDbFieldContent, ref string strDbFieldVal)
        {
            strDbFieldContent = pi.GetValue(data, null).ToMyFieldString();
            strDbFieldVal = strDbFieldContent.GoOriginal();
        ...
        private static void AppandInsertSection(ColumnDef colDef, string FieldContent, ref string strInsert, ref string strValue)
        {
            ...
            if (!string.IsNullOrEmpty(FieldVal))
            {
                if (strInsert.Length > 0)
                    strInsert += ",";
                strInsert += string.Format("`{0}`", colDef.Name);
        ...
        /// <param name="strInsert"></param>
        /// <param name="strValue"></param>
        private static void AppandInsertSection<TKey, TValue>(Dictionary<TKey, TValue> SourceDic, ref List<string> LstInsert, ref List<string> LstValue)
        {
            if (SourceDic == null)
                return;
            foreach (TKey k in SourceDic.Keys)
            {
                ...
                {
                    TValue value = SourceDic[k];
                    string strKey = k.ToMyString();
                    string strValue = value.ToMyString();
                    if (!LstInsert.Contains(strKey))
                    {
                        LstInsert.Add(strKey);
                        if (typeof(TValue) == typeof(double) || typeof(TValue) == typeof(float) || typeof(TValue) == typeof(int))
                            LstValue.Add(string.Format("{0}", strValue));
                        else
                            LstValue.Add(string.Format("'{0}'", strValue.ValueFilter()));
                    }
                }
            }
        }
                ...
                if (k is string)
                {
                    ...
                    if (IsWhereField)
                    {
                        if (strWhere.Length > 0)
                            strWhere += " and ";
                        if (FieldContent.CheckIfExpress())
                            strWhere += FieldContent.ParseExpress();
                        else
                            strWhere += string.Format("`{0}`='{1}'", k, FieldContent.ParseWhere());
                    }
                    else
                    {
                        if (strUpdateExpress.Length > 0)
